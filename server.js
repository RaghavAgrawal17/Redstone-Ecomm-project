...
app.get("/search",(req,res) => {
    ...
    Product.find({ "_id": query.id}).then((result)=>{
        console.log(result)
        res.render("Productpage",{name: req.user.name, email: req.user.email, res:result, count: count});
    }).catch((err)=>{
    ...
    if (errors.length > 0) {
        ...
        User.findOne({ email: email }).then(user => {
            if (!user) {
                ...
                const CLIENT_URL = 'http://' + req.headers.host;
                const output = `${CLIENT_URL}/forgot/${token}`;
                ...
                User.updateOne({ resetLink: token }, (err, success) => {
                    if (err) {
                        ...
                        // send mail with defined transport object
                        const mailOptions = {
                            from: '"Do Not Reply" <raghavservermail25@gmail.com>', // sender address
                            to: email, // list of receivers
                            subject: "Account Password Reset: Auth ✔", // Subject line
                            html:"<h2>Please click on below link to reset your account password</h2>"+
                            "<a href='"+output+"'>Reset Your Account Password</a>"+
...
app.post('/reset/:id',async function(req, res){
